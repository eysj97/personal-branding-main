...
export default function AquaplanetSpread({ stacked }) {
  return (
    <SpreadPalette panel="rgba(255,255,255,0.1)" ink="#ffffff">
      ...
      <div className={`${COLUMN} w-[474px] gap-[71px] self-start pb-[50px]`}>
        ...
        <div
          className={`flex flex-col gap-[8px] font-['Pretendard'] tracking-[-0.44px] text-white ${
            stacked ? "mx-auto w-[350px] items-start text-left" : "mb-[39px] items-end"
          }`}
        >
          ...
          <p
            className={`w-full text-[22px] font-semibold leading-[1.2] ${
        ...
        <Block>
          ...
          <Body size={18} tracking="-0.36px">
            {[
              "할인 정보가 프로모션·기획상품·제휴 혜택으로 흩어져 있어 한눈에 파악하기 어려웠습니다. 먼저 흩어진 정보를 모두 찾아 성격별로 정리했습니다.",
              "지점마다 요금이 달라 한 화면에 다 담으면 오히려 읽기 어려워, 지점 단위로 나누되 지점을 바꾸면 화면 전체가 아니라 요금 카드만 바뀌도록 했습니다.",
            ]}
          </Body>
        </Block>
